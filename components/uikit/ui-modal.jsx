...
export function UiModal({ width = "md", className, children, isOpen = false, onClose}) {
  ...
  const modal = (
    <div onClick={handleClick} className={clsx(className, "fixed inset-0 bg-slate-900/60 backdrop-blur pt-10 pb-10 overflow-auto")}>
      <div data-id='modal'
        className={clsx(
          "bg-white rounded-lg min-h-[320] relative flex flex-col",
          {
            md: "max-w-[640] w-full mx-auto",
            full: "mx-5",
          }[width],
        )}
      >
        <button className="w-8 h-8 flex items-center justify-center bg-white/10 absolute top-0 left-[calc(100%+12px)] rounded-sm cursor-pointer hover:bg-white/40 transition-colors" onClick={onClose}>
          <CrossLightIcon className="w-4 h-4 text-white"></CrossLightIcon>
        </button>
        {children}
      </div>
    </div>
...
UiModal.Footer = function UiModalFooter({children, className}) {
  return (
    <div className={clsx(className, "flex p-6 gap-4 justify-end mt-auto")}>{children}</div>
  )
}
